function [maxdisplacement pathlength meanspeed instantspeed displacement] = CT_displace_v4(pt,xvals, yvals)
...
B= ~isnan(displacement);
Indices = arrayfun(@(x) find(B(:,x),1,'last'), 1:size(displacement,2));
finaldisp = arrayfun(@(x,y) displacement(x,y), Indices, 1:size(displacement,2));
